...
public class Control {
    ...
    public void RunInstruction(IR IRobject , RF RFtable, XF XFtable, Memory Mem, ALU ALU){
    ...
    public void AMR(IR IRobject , RF RFtable, XF XFtable, Memory Mem, ALU ALU){
        ...
        ALU.Calculate(RFtable.getSwitch(IRobject.getRFI1()), MDR.getData(), IRobject.getOpCode(), RES);
    ...
    public void AIR(IR IRobject , RF RFtable, ALU ALU){
        //if immed is zero no need to change register but the value is reinserted into
        //the register because the single step mode only works when the set functions are called
        if(IRobject.getImmed()==0){
             RFtable.setSwitch(IRobject.getRFI1(), RFtable.getSwitch(IRobject.getRFI1()));
        }
        //if the General Register is empty than just copy and paste the immed into the register
        if(RFtable.getSwitch(IRobject.getRFI1())==0){
            RFtable.setSwitch(IRobject.getRFI1(), IRobject.getImmed());
        }
        //else add the register to the immed and put back the value in the register
        else{
            ALU.Calculate(RFtable.getSwitch(IRobject.getRFI1()), IRobject.getImmed(), IRobject.getOpCode(), RES);
            RFtable.setSwitch(IRobject.getRFI1(), RES.getData());
        }    
    }
    ///
